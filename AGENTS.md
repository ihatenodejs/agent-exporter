...
Agent Exporter is a CLI tool for tracking and analyzing LLM usage costs across multiple AI agent providers (OpenCode, Claude Code/CCUsage, Codex, Gemini, Qwen, Kimi CLI). It uses SQLite for storage, calculates costs using @pydantic/genai-prices with a fallback pricing database, and exports data in various formats.
